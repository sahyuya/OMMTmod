...
object UploadClient {
  ...
  fun initialize() {
    ...
    initialized = true
    ClientReceiveMessageEvents.GAME.register { message, _ -> receive(message.string) }
  }
  ...
  fun checkAvailability() {
    ...
    val connection = MinecraftClient.getInstance().networkHandler
        ?: run { status = "Upload cancelled: not connected to a server"; return }
  ...
  fun status(): String {
    ...
    if (!ready && requestId != null && helloAt != 0L && System.currentTimeMillis() - helloAt > HELLO_TIMEOUT_MS) {
      ...
      else { commands.clear(); requestId = null; helloAt = 0L; status = "OyasaiMusic upload is unavailable" }
    }
    return status
  }
  ...
  fun upload(oymi: ByteArray) {
    if (oymi.size !in 1..MAX_OYMI) { status = "OYMI must be at most 1 MiB"; return }
    try { validateOymiHeader(oymi) }
    catch (error: IllegalArgumentException) { status = "Upload rejected: ${error.message}"; return }
    ...
    totalChunks = chunks.size; sentChunks = 0; pending = Prepared(id, if (unicode15) "u" else "a", chunks, encoded.length, compressed.size, oymi.size, hash43(oymi), compact.size, validateOymiCount(oymi), oymi.copyOf())
    status = "Waiting for OyasaiMusic upload capability…"
  }

  private data class Prepared(val id: String, val encoding: String, val chunks: List<String>, val encoded: Int, val compressed: Int, val oymi: Int, val hash: String, val transport: Int, val notes: Int, val canonicalOymi: ByteArray)
  ...
  private fun receive(raw: String) {
    ...
    if (words.size < 4 || words[0] != "OMMT" || words[2] != requestId) return
    if (words[1] !in setOf("UPLOAD1", "UPLOAD2")) return
    when (words[3]) {
      "READY" -> {
        if (ready) return
        ...
        if (words[1] == "UPLOAD2" && protocol == 2) {
          ...
          if (wanted !in capabilities) { abortLocal("OyasaiMusic does not advertise required upload encoding"); return }
        }
        ready = true
        val prepared = pending
        if (prepared == null || prepared.id != requestId) { status = "OyasaiMusic upload ready"; return }
        if (words[1] == "UPLOAD2" && protocol == 2) {
          ...
          val pieces = ascii.chunked(200); if (pieces.size !in 1..MAX_CHUNKS) { abortLocal("Legacy upload needs too many chunks"); return }
      ...
      "DONE" -> { status = "Imported as private draft #${words.getOrNull(4) ?: "?"}"; clearCompleted() }
      "ERROR" -> {
        ...
        else { status = "Upload failed: ${words.getOrNull(4) ?: "server error"}"; clearCompleted() }
      }
    }
  }
  ...
  private fun abortLocal(message: String) { commands.clear(); pending = null; requestId = null; ready = false; importing=false; helloAt = 0L; nextSendAt = 0L; helloConnection = null; status = message }
  ...
  private fun validateOymiHeader(bytes: ByteArray) {
    require(bytes.size in 20..MAX_OYMI) { "OYMI must be 20 bytes to 1 MiB" }
    val input = ByteBuffer.wrap(bytes)
    require(input.int == 0x4F594D49 && input.short.toInt() == 1 && input.short.toInt() == 0) { "invalid OYMI v1 header" }
